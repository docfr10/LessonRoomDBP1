...
class DB(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun getAll(order: String): List<Result> {
        ...
        while (cursor.moveToNext()) {
            allRecords.add(
                Result(
                    cursor.getString(cursor.getColumnIndexOrThrow(DBContract.Entry.COLUMN_NAME_NAME)),
                    cursor.getInt(cursor.getColumnIndexOrThrow(DBContract.Entry.COLUMN_NAME_RESULT))
                )
            )
        }
        return allRecords
